...
library("readxl")
uti.KP =  read_excel("UTI.xlsx", sheet = 3)
uti.KP$iso = uti.KP$Isolates_KP
uti.KP$resis = uti.KP$Resistance_KP
uti.KP$freq = uti.KP$resis/uti.KP$iso
...
ll <- function(param){
  r0 = param[1]
  rho = param[2]
  R <- dbinom(kp.k, kp.n, exp((rho*uti.KP$consump+kp.theta -1)*t)/(1/r0-1+exp((rho*uti.KP$consump + kp.theta-1)*t)))
  -sum(log(R))
}
kp.rho = optim(par = c(0.0005,0), ll)$par[2]

kp.r0 = optim(par = c(0.0005,0), ll)$par[1]
kp.rt <- exp((kp.rho*uti.KP$consump+kp.theta -1)*t)/(1/kp.r0-1+exp((kp.rho*uti.KP$consump + kp.theta-1)*t))
cbind(kp.rho, kp.r0)
plot(kp.rt,  ylab = "r(t)", main = "Fitted r(t)-UTI.KP",x = c(2000:2016),type = "l")
data.frame(kp.rt, uti.KP$freq)
```

```{r}
consump <- lm(consump ~ t, data = uti.KP)
...
plot(2000:2040, ylim = c(min(uti.KP$freq), max(rt.trend.g)),y = c(uti.KP$freq, rt.trend1, rt.trend2.1, rep(rt.trend2.1[5],15)), type = "l", xlab = "Year", ylab = "Resistance Frequency",main = "Projected Resistance Frequency with/out Bacteria Stewardship", col = "blue", lty = 2)
par(new = T)
plot(2000:2040, ylim = c(min(uti.KP$freq), max(rt.trend.g)),y = c(uti.KP$freq, rt.trend1, rt.trend2, rt.trend3.1, rep(rt.trend3.1[5],10)), type = "l", xlab = "Year", ylab = "Resistance Frequency",main = "Projected Resistance Frequency with/out Bacteria Stewardship", col = "purple", lty = 2)
par(new = T)
plot(2000:2040, ylim = c(min(uti.KP$freq), max(rt.trend.g)),y = c(uti.KP$freq, rt.trend1, rt.trend2, rt.trend3, rt.trend4.1, rep(rt.trend4.1[5],5)), type = "l", xlab = "Year", ylab = "Resistance Frequency",main = "Projected Resistance Frequency with/out Bacteria Stewardship", col = "red", lty = 2)
par(new = T)
plot(2000:2040, ylim = c(min(uti.KP$freq), max(rt.trend.g)),y = c(uti.KP$freq,rt.trend.g), type = "l", xlab = "Year", ylab = "Resistance Frequency",main = "Projected Resistance Frequency with/out Bacteria Stewardship")
...
uti.kp = uti.KP$`UTI-KP`[17]
cbp.uti = mean(uti.KP$`CBP-cUTIs`)
...
cn = c()
for(i in 17:40){
  cn = uti.kp*cbp.uti*pop$Population[(i-14)]*uti.inc
  case = c(case,cn)
}
...
axis(side=2, at=c(0, 2000, 4000, 6000,8000), hadj=0.8, cex.axis=0.9, las=2)
legend(2019, 8000, legend = c("without stewardship", "2020", "2025", "2030"), col = c("black", "blue", "purple", "red"), lty = c(1,2,2,2),cex=0.8, text.font=4, bg='lightblue')
```
...
```{r}
in.cbp = 0.122622696
cbp.in.prescrip = ceiling(uti.inc*uti.kp*pop$Population[c(-1,-2)]*in.cbp*cbp.uti)
...
axis(side=2, at=c(0, 200, 400, 600,800, 1000, 1200), hadj=0.8, cex.axis=0.9, las=2)
legend(2019, 400, legend = c("no stewardship", "2020", "2025", "2030"), col = c("black", "blue", "purple", "red"), lty = c(1,2,2,2),cex=0.8, text.font=4, bg='lightblue')
```
